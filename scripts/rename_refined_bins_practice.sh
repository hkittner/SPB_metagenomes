...
for REF_DIR in "${TOPDIR}"/bin_refinement_*; do
  ...
  for f in bin.*.fa; do
      ...
      # cut -d. -f2 = split string by delimiter . and take field 2, then assign to variable num
      num=$(echo "$f" | cut -d. -f2)
      # creates the new sample name including the sample and bin number
      new="${SAMPLE}_coassembly_bin.${num}.fa"
      # echo logs what's happening. mv renames bin.*.fa to the above variable
      echo "Renaming $f -> $new"
      mv "$f" "$new"
  done
  
  # move back to the bin directory
  cd "$TOPDIR"
done
