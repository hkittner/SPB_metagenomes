...
for tbl in "$rdhA_hits_dir"/*-proteins_rdhA_hits.tbl; do
    ...
    while read -r orf_id evalue; do
        # Extract full sequence (without extra description lines)
        seq=$(awk -v id="$orf_id" 'BEGIN {found=0} 
            /^>/ {if (found) exit; if ($0 ~ id) {found=1; sub(/^>/, ""); print $1}} 
            found && !/^>/ {print}' "$faa_file")

        # Only add to output if a valid sequence was found
        if [[ -n "$seq" ]]; then
            header=">$bin_name|rdhA|$orf_id|E=$evalue"
            echo "$header" >> "$output_faa"
            echo "$seq" | awk '{printf "%s", $0} END {print ""}' >> "$output_faa"  # Print on a single line
        else
